...
class AppViewModel: ObservableObject {
    ...
    func continueDrag(at point: CGPoint) {
        if selectedBlockIds.count > 1 {
            // 多选 → 整体移动：以 activeBlockId 为参考块
            let refId = activeBlockId ?? selectedBlockIds.first!
            guard let refIdx = canvasState.blocks.firstIndex(where: { $0.id == refId }) else { return }
            let dx = (point.x - dragOffset.x) - canvasState.blocks[refIdx].x
            let dy = (point.y - dragOffset.y) - canvasState.blocks[refIdx].y
